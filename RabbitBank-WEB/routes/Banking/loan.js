...
router.get('/', checkCookie, function (req, res, next) {          // 대출 페이지 불러오기
    ...
    profile(cookie).then(pending => {
        ...
        axios({          // 대출 페이지 불러오기를 위한 api로 req
            ...
            <a onclick="document.getElementById('get_debt').submit()" class="btn btn-user btn-block" id="submitbutton" style="background-color:#f00 !important; color:white !important;">
            대출
            </a>
            `;
