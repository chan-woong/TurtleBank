...
router.get('/', checkCookie, function (req, res) {          // 마이데이터(신청 전) 화면 불러오기
    ...
    profile(cookie).then(profileData => {
        ...
        <form action="/bank/mydata_auth" method="post">
            <button class="btn btn-user btn-block" type="submit" id="view" value="submit" style="background-color:#f00 !important; color:white !important;">마이데이터 요청</button>
        </form>
    </div>
            `
    ...
    profile(cookie).then(profileData => {
        axios({          // 마이데이터 연동을 위한 API로 request
            ...
            if(result.status.code==200){          // user의 마이데이터 연동요청 후 인증번호가 보내졌을 경우
              ...
              <a onclick="document.getElementById('authnum').submit()" class="btn btn-user btn-block" style="background-color:#f00 !important; color:white !important;">
              확인
            </a>
            <br>
            <a href="/bank/mydata_auth" onclick="document.getElementById('register').submit();" class="btn btn-user btn-block" style="background-color:#f00 !important; color:white !important;">
              취소
            </a>
            `
                return res.render("Banking/mydata_auth", {html_data: result, pending: profileData, select: "mydata"});
