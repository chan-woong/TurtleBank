        ...
        axios({          // B은행에 있는 account를 불러오기 위한 api로 request
            ...
            if(account_list.length > 0) {          // B은행에서 받아온 data(user 계좌정보)가 존재하는 경우
                ...
                account_list.forEach(account => {
                    ...
                    result += "class='btn btn-user btn-block' type='button' onclick='redirectToTransferPage(\"" + account.account_number + "\", \"" + account.balance + "\", \"" + account.bank_code + "\")' style='background-color:#f00 !important; color:white !important;'>"
                    result += "송금</button></td>";
                    result += "</tr>\n";
