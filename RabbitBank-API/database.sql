drop database if exists dvba_sword;
create database if not exists dvba_sword;
use dvba_sword;
...
INSERT INTO `account`
values (999999, 333, "admin", 100000000);


drop database if exists board_sword;
create database if not exists board_sword;
use board_sword;
...
grant all privileges on dvba_sword.* to api@'%';
grant all privileges on board_sword.* to web@'%';
